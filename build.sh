...
echo "Pip installing packages using local compilation for needed packages..." > /dev/null 2>&1
/usr/bin/pip-${PYTHON_VER} install --upgrade pip==9.0.3 setuptools
/usr/bin/pip-${PYTHON_VER} install --no-binary transformers transformers==${TRANSFORMERS_VER}
/usr/bin/pip-${PYTHON_VER} install --target $SITE_PACKAGES_DIR flask
/usr/bin/pip-${PYTHON_VER} install --target $SITE_PACKAGES_DIR torch
/usr/bin/pip-${PYTHON_VER} install --target $SITE_PACKAGES_DIR zappa
/usr/bin/pip-${PYTHON_VER} install --target $SITE_PACKAGES_DIR awscli
